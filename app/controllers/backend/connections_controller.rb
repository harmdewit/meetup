class Backend::ConnectionsController < Backend::AdminController
    ...
    if @connection1.save && @connection2.save
      ...
      redirect_to( backend_connections_url, :notice => "#{@participant.user.first_name} #{@participant.user.last_name} is nu aanbevolen aan #{@connected_participant.user.first_name} #{@connected_participant.user.last_name}.")
    end
	end
end
